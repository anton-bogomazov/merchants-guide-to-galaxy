...
class GalaxyNumeralTest : FreeSpec({
    ...
    "impossible to create digit from blank string" - {
        withData(
            ...
            GalaxyNumeral.from(it)
                .shouldBeLeft(GalaxyNumeralValidationError.EmptyString)
        }
    }
    ...
    "impossible to create digit from non-letter string" - {
        withData(
            ...
            GalaxyNumeral.from(it)
                .shouldBeLeft(GalaxyNumeralValidationError.NonLetters)
        }
    }

    "impossible to create digit from string with more than one word" - {
        GalaxyNumeral.from("foo bar")
            .shouldBeLeft(GalaxyNumeralValidationError.MoreThanOneWord)
    }
})
